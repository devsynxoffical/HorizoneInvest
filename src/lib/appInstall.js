...
export function getApkDownloadUrl() {
  ...
  return `${normalizedBase}horizoneinvest.apk`
}

export function downloadApk(url = getApkDownloadUrl()) {
  const link = document.createElement('a')
  link.href = url
  link.download = url.split('/').filter(Boolean).pop() || 'horizoneinvest.apk'
...
export const APP_INSTALL_REQUEST_EVENT = 'horizone:install-app-request'
  ...
  if (isIos()) {
    return {
      ...
      steps: [
        ...
        'Tap "Add" — HorizonInvest will open like an app.',
      ],
    }
  }
  if (isAndroid()) {
